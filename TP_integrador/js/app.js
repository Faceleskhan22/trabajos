...
function aplicarFiltros() {
    ...
    const favoritosIds = obtenerFavoritos();
     console.log("Productos en memoria:", productosEnMemoria);
    let filtro = filtrarProductos(productosEnMemoria, busqueda, categoriaActual, favoritosIds );
    totalProductosSpan.textContent = filtro.length;
    const totalPrecio = calcularTotalCatalogo(filtro);
    ...
    if(filtro.length === 0){
        ...
        contenedorCatalogo.innerHTML= filtro.map(producto => crearTarjetaProductoHTML(producto, esProductoFavorito(producto.id))).join("");
...
async function cargarCatalogo() {
    ...
    try{
        ...
        console.log("Catalogo descargado con exito, cantidad:", productosEnMemoria.length );
